...
function formatTopicList(data) {
    ...
    for (let item of data) {
        dataHTML += `<br>` + `<div class="topic"><span class="topicTitle">${item.topic}</span><br>` +
            `<br>` +
            `<span class="topicDescription">${item.topicDescription}</span>` +
            `<button class="goToTopic btn btn-lg btn-dark" data-topic="${item.topic}">Go to Quizzes</button></div>` +
            `<br>`;
    }
